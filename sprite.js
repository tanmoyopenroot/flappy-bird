...
function Sprite(img, x, y, s_width, s_height) {
  this.img = img;
  this.x = x * 2;
  this.y = y * 2;
  this.width = s_width * 2;
  this.height = s_height * 2;
};

Sprite.prototype.draw = function(x, y) {
  image(this.img, x, y, this.width, this.height, this.x, this.y, this.width, this.height);
};
...
function initSprites(img) {
  ...
  s_medal = [
    new Sprite(img, 173, 114, 25, 22),
    new Sprite(img, 173, 137, 25, 22)
  ];
  ...
  s_numberS.draw = s_numberB.draw = function(x, y, num, scoreBoard = false) {
    // num = num.toString();
    ...
    for (var i = 0; i < num_str.length; i++) {
      ...
      if (!scoreBoard) 
        x = width / 2 - (num_str.length - i) * step + this.width;

      num_int = parseInt(num_str[i]);
      image(this.img, x, y, this.width, this.height, num_int * step, this.y, this.width, this.height);
      // console.log(num_int);
    }
  }
}
